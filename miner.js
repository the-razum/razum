...
// ---------- Run task on local Ollama with streaming ----------
async function runTask(task) {
  ...
  const flushChunk = async () => {
    ...
    try {
      ...
      // Non-fatal: chunk may be lost but final result will be submitted
      err('  chunk send failed:', e.message)
    }
  }
      ...
      for (const line of lines) {
        try {
          ...
          if (obj.message?.content) {
            ...
            chunkBuffer += obj.message.content
          }
          if (obj.done) {
            done = true
